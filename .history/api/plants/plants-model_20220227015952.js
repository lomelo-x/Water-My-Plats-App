...
async function updatePlant(user, plant_id) {
	const [updatedUser] = await db('plants')
		...
		.update(user, ['plant_id', 'plant_name', 'species', 'h20Frequency', uploaded_image]);
	return updatedUser;
}
...
async function deletePlant(plant_id) {
	const [deletedUser] = await db('plants')
		.where('plant_id', plant_id)
		.delete(['plant_id', 'plant_name', 'species', 'h20Frequency', uploaded_image]);
	return deletedUser;
}
